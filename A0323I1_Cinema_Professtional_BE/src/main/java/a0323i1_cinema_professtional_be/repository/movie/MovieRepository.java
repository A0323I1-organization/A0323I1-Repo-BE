...
public interface MovieRepository extends JpaRepository<Movie, Integer> {
    ...
    @Query(value = "select m.movieId as movieId, m.movieName as movieName, m.movieImage as movieImage, m.movieStartDay as movieStartDay from Movie as m where m.movieStartDay <= current_date() order by m.movieStartDay desc")
    List<MovieProjection> findAllMovieIsShowing();

    @Query(value = "select m.movieId as movieId, m.movieName as movieName, m.movieImage as movieImage, m.movieStartDay as movieStartDay from Movie as m where m.movieStartDay > current_date() order by m.movieStartDay asc")
