...
public interface SeatRepository extends JpaRepository<Seat, Integer> {
    ...
    @Query(value = "select s.seatName as seatName, ts.typeSeatPrice as typeSeatPrice from Seat as s " +
            "join TypeSeat as ts " +
            " on ts.typeSeatId= s.typeSeat.typeSeatId where s.seatId =:id")
    List<SeatDTO> getSeatById(@Param("id") int id);
